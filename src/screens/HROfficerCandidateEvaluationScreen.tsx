...
interface RankedCandidate {
  ...
  applied_at: string;
  status: string;
  fit_percentage: number;   // AI SFIA fit score
  survey_score: number;     // HR survey score — shown in manual mode
  sfia_rank: number;
  manual_rank: number;
  pillars: SFIAPillar[];
}
...
function generateMockCandidates(jobId: string): RankedCandidate[] {
  ...
  const candidates = MOCK_NAMES.map(([first, last], i) => {
    ...
    // (backend): survey_score from GET /jobs/{job_id}/candidates/ranked
    const surveyVariance = ((i * 11 + seed * 3) % 21) - 10;
    const survey_score = Math.max(30, Math.min(100, fit + surveyVariance));
  ...
  return (
    <View style={styles.pillarWrap}>
        ...
        return (
          <View key={p.name} style={styles.pillarRow}>
            <View style={styles.pillarLabelRow}>
              ...
              <Text style={[styles.pillarScore, { color: hasGap ? "#DC2626" : "#16A34A" }]}>{p.supply} / {p.demand}</Text>
            </View>
      ...
      <View style={styles.legendRow}>
        <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: "#94A3B8" }]} /><Text style={styles.legendText}>Company Demand</Text></View>
        <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: "#4ADE80" }]} /><Text style={styles.legendText}>Supply (met)</Text></View>
        <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: "#F87171" }]} /><Text style={styles.legendText}>Supply (gap)</Text></View>
      </View>
    </View>
  );
}

function CandidateCard({ candidate, rank, mode }: { candidate: RankedCandidate; rank: number; mode: RankingMode }) {
  ...
  // ── Key change: show survey_score in manual mode, fit_percentage in SFIA mode
  const displayScore = mode === "manual" ? candidate.survey_score : candidate.fit_percentage;
  const scoreLabel = mode === "manual" ? "Survey" : "Fit";
    ...
    <View style={styles.candidateCard}>
      <View style={styles.candidateRow}>
        ...
        <View style={styles.candidateInfo}>
          <Text style={styles.candidateName} numberOfLines={1}>{candidate.first_name} {candidate.last_name}</Text>
          <Text style={styles.candidateEmail} numberOfLines={1}>{candidate.email}</Text>
        </View>
        <View style={styles.fitWrap}>
          <Text style={[styles.fitScore, { color: fitColor(displayScore) }]}>{displayScore}%</Text>
          <Text style={[styles.fitLabel, mode === "manual" && { color: "#D97706" }]}>{scoreLabel}</Text>
        </View>
      ...
      <View style={styles.statusRow}>
        ...
        {/* In manual mode show SFIA score for comparison */}
        {mode === "manual" && (
          <View style={styles.sfiaCompare}>
            <Text style={styles.sfiaCompareText}>SFIA: {candidate.fit_percentage}%</Text>
          </View>
        )}
      </View>
...
export const HROfficerCandidateEvaluationScreen = ({ route, navigation }: any) => {
  ...
  const avgFitTop20 = Math.round(sfiaSorted.slice(0, 20).reduce((s, c) => s + c.fit_percentage, 0) / 20);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.layout}>
        {!isMobile && (
          <Sidebar role="hr" userName={session.name} email={session.email} activeScreen="CandidateEvaluation" navigation={navigation} />
        )}
        <View style={styles.main}>
          {isMobile && (
            <MobileRoleMenu role="hr" userName={session.name} email={session.email} activeScreen="CandidateEvaluation" navigation={navigation} />
          )}
          <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <GradientHero style={styles.heroCard}>
              ...
              <Text style={styles.heroSubtitle}>
                SFIA mode shows AI fit scores. Manual mode shows Survey Scores.
              </Text>
              <View style={styles.heroStatsRow}>
                ...
                <View style={styles.heroStat}>
                  <Text style={[styles.heroStatValue, { color: "#34D399" }]}>{sfiaSorted[0]?.fit_percentage ?? 0}%</Text>
            ...
            <View style={styles.section}>
              ...
              <Pressable style={styles.jobSelectorBtn} onPress={() => setJobDropdownOpen((v) => !v)}>
                ...
                <View style={styles.dropdown}>
                  {MOCK_JOBS.map((job) => (
                    <Pressable
                      ...
                      style={[styles.dropdownItem, job.job_posting_id === selectedJob.job_posting_id && styles.dropdownItemActive]}
                      onPress={() => handleJobSelect(job)}
                    >
                      <Text style={styles.dropdownItemTitle}>{job.title}</Text>
                      <Text style={styles.dropdownItemSub}>{job.department} · {job.total_applicants} applicants</Text>
                    </Pressable>
                  ))}
                </View>
              )}
            ...
            <View style={styles.podiumRow}>
              {top3.map((c, i) => (
                <View key={c.application_id} style={styles.podiumCard}>
                  ...
                  <Text style={styles.podiumName} numberOfLines={1}>{c.first_name} {c.last_name}</Text>
                  <Text style={[styles.podiumScore, { color: fitColor(mode === "manual" ? c.survey_score : c.fit_percentage) }]}>
                    {mode === "manual" ? c.survey_score : c.fit_percentage}%
                  </Text>
                  <Text style={styles.podiumScoreLabel}>{mode === "manual" ? "Survey" : "Fit"}</Text>
                </View>
              ))}
            </View>
            ...
            <View style={styles.listCard}>
              <View style={styles.listToolbar}>
                <View>
                  <Text style={styles.listTitle}>{showAll ? `All ${activeList.length} Candidates` : "Top 20 Candidates"}</Text>
                  <Text style={styles.listSubtitle}>
                    {mode === "sfia" ? "Showing AI Fit Score" : "Showing Survey Score"}
                  </Text>
                </View>
                <View style={styles.modeToggle}>
                  <Pressable style={[styles.modeBtn, mode === "sfia" && styles.modeBtnActive]} onPress={() => setMode("sfia")}>
                    <Text style={[styles.modeBtnText, mode === "sfia" && styles.modeBtnTextActive]}>SFIA</Text>
                  </Pressable>
              ...
              {mode === "manual" && (
                <View style={styles.manualBanner}>
                  <Feather name="edit-2" size={13} color="#92400E" style={{ marginRight: 6 }} />
                  <Text style={styles.manualBannerText}>Showing Survey Scores instead of AI Fit %.</Text>
              ...
              <View style={styles.candidateList}>
                {visibleList.map((candidate, index) => (
                  <CandidateCard key={candidate.application_id} candidate={candidate} rank={index + 1} mode={mode} />
                ))}
              </View>

              {activeList.length > 20 && (
                <Pressable style={styles.showAllBtn} onPress={() => setShowAll((v) => !v)}>
                  <Text style={styles.showAllText}>{showAll ? "Show Top 20 Only" : `Show All ${activeList.length} Candidates`}</Text>
              ...
              <View style={styles.listFooter}>
                <Text style={styles.listFooterText}>
                  Showing {visibleList.length} of {activeList.length} · {mode === "manual" ? "Survey Score" : "SFIA Fit"} mode
                </Text>
              </View>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  podiumScoreLabel: { color: "#94A3B8", fontSize: 9, fontWeight: "700", textTransform: "uppercase" },
  ...
  modeToggle: { flexDirection: "row", backgroundColor: "#F1F5F9", borderRadius: 10, padding: 3, gap: 2 },
  modeBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  modeBtnActive: { backgroundColor: "#FFFFFF", shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 2 },
  modeBtnText: { color: "#64748B", fontSize: 12, fontWeight: "700" },
  modeBtnTextActive: { color: "#1E3A8A" },
  manualBanner: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFFBEB", borderBottomWidth: 1, borderBottomColor: "#FDE68A", paddingHorizontal: 16, paddingVertical: 10 },
  ...
  statusRow: { flexDirection: "row", alignItems: "center", marginTop: 8, paddingLeft: 42, gap: 8 },
  statusPill: { borderWidth: 1, borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3 },
  statusText: { fontSize: 10, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.5 },
  sfiaCompare: { backgroundColor: "#EFF6FF", borderWidth: 1, borderColor: "#BFDBFE", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3 },
  sfiaCompareText: { color: "#1E3A8A", fontSize: 10, fontWeight: "700" },
